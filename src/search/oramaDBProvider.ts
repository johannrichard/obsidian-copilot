...
export class OramaDBProvider implements DBProvider {
  private static oramaDb: Orama<any> | undefined; // Singleton instance
  ...
  constructor(private app: App) {
    ...
    subscribeToSettingsChange(async () => {
      ...
      if (Platform.isMobile && settings.disableIndexOnMobile) {
        ...
        OramaDBProvider.oramaDb = undefined;
      } else if (Platform.isMobile && !settings.disableIndexOnMobile && !OramaDBProvider.oramaDb) {
    ...
    try {
      ...
      try {
        if (await this.chunkedStorage.exists()) {
          OramaDBProvider.oramaDb = await this.chunkedStorage.loadDatabase();
          logInfo("Loaded existing chunked Orama database from disk.");
          return OramaDBProvider.oramaDb;
        }
      } catch (error) {
  ...
  async saveDB() {
    ...
    if (!OramaDBProvider.oramaDb || !this.chunkedStorage) {
    ...
    try {
      await this.chunkedStorage.saveDatabase(OramaDBProvider.oramaDb);
  ...
  async recreateIndex(embeddingInstance: Embeddings | undefined): Promise<void> {
    try {
      // Ensure database is initialized first
      if (!OramaDBProvider.oramaDb) {
      ...
      OramaDBProvider.oramaDb = await this.createNewDb(embeddingInstance);
  ...
  public async removeDocs(filePath: string) {
    ...
    try {
      const searchResult = await search(OramaDBProvider.oramaDb, {
        term: filePath,
        properties: ["path"],
      });
      if (searchResult.hits.length > 0) {
        await removeMultiple(
          OramaDBProvider.oramaDb,
  ...
  public async getDocsByPath(path: string): Promise<any[]> {
    if (!OramaDBProvider.oramaDb) throw new Error("DB not initialized");
    if (!path) return [];
    const result = await search(OramaDBProvider.oramaDb, {
      term: path,
      properties: ["path"],
      exact: true,
      includeVectors: true,
  ...
  async upsert(docToSave: any): Promise<any | undefined> {
    if (!OramaDBProvider.oramaDb) throw new Error("DB not initialized");
    const db = OramaDBProvider.oramaDb;
  ...
  async checkAndHandleEmbeddingModelChange(embeddingInstance: Embeddings): Promise<boolean> {
    ...
    const singleDoc = await search(OramaDBProvider.oramaDb, {
      term: "",
      limit: 1,
    });
  ...
  public async garbageCollect(): Promise<number> {
    ...
    try {
      ...
      if (docsToRemove.length === 1) {
        await remove(OramaDBProvider.oramaDb, docsToRemove[0].id);
      } else {
        await removeMultiple(
          OramaDBProvider.oramaDb,
  ...
  public async hasIndex(notePath: string): Promise<boolean> {
    ...
    const docs = await this.getDocsByPath(notePath);
